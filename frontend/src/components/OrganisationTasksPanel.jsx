...
const emptyForm = {
  assignedToId: '', title: '', description: '', complaintReference: '', priority: 'medium',
  area: '', address: '', city: '', district: '', state: '', pincode: '', lat: '', lng: '',
}
...
export default function OrganisationTasksPanel({ organisation }) {
  ...
  return <section><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-orange-700">Field operations</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight text-navy">Tasks</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Assign complaint work to active staff and review completed before-and-after evidence.</p></div><button onClick={() => setShowForm((value) => !value)} className="rounded-xl bg-navy px-5 py-3 text-sm font-bold text-white">{showForm ? 'Close form' : '+ Assign task'}</button></div>
    ...
    {showForm && <form onSubmit={submit} className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="font-extrabold text-navy">New staff assignment</h2><div className="mt-5 grid gap-5 sm:grid-cols-2"><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Assign to staff *</span><select required name="assignedToId" value={form.assignedToId} onChange={change} className={inputClass}><option value="">Select staff member</option>{staff.map((member) => <option key={member.user.id} value={member.user.id}>{member.user.profile?.fullName || member.user.email}</option>)}</select></label><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Priority</span><select name="priority" value={form.priority} onChange={change} className={inputClass}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><label className="block sm:col-span-2"><span className="mb-2 block text-sm font-bold text-slate-700">Task title *</span><input required name="title" maxLength="140" value={form.title} onChange={change} placeholder="Example: Restore drinking water supply" className={inputClass} /></label><label className="block sm:col-span-2"><span className="mb-2 block text-sm font-bold text-slate-700">Task details *</span><textarea required name="description" rows="4" maxLength="2000" value={form.description} onChange={change} placeholder="Explain the complaint and the expected resolution." className={inputClass} /></label><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Complaint reference</span><input name="complaintReference" value={form.complaintReference} onChange={change} placeholder="CMP-2026-001" className={inputClass} /></label><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Area / locality *</span><input required name="area" value={form.area} onChange={change} placeholder="Ward 12, Sector 4" className={inputClass} /></label><label className="block sm:col-span-2"><span className="mb-2 block text-sm font-bold text-slate-700">Full work address *</span><input required name="address" value={form.address} onChange={change} placeholder="House/street/landmark" className={inputClass} /></label>{['city', 'district', 'state', 'pincode'].map((field) => <label key={field} className="block"><span className="mb-2 block text-sm font-bold capitalize text-slate-700">{field}</span><input name={field} value={form[field]} onChange={change} className={inputClass} /></label>)}<label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Latitude (optional)</span><input name="lat" type="number" step="any" min="-90" max="90" value={form.lat} onChange={change} className={inputClass} /></label><label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">Longitude (optional)</span><input name="lng" type="number" step="any" min="-180" max="180" value={form.lng} onChange={change} className={inputClass} /></label></div><div className="mt-6 flex justify-end"><button disabled={saving || staff.length === 0} className="rounded-xl bg-india-green px-6 py-3 text-sm font-bold text-white disabled:opacity-50">{saving ? 'Assigning…' : 'Assign task'}</button></div></form>}
