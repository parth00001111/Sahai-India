...
function TaskDetail({ task, onUpdated }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="grid gap-4 border-b border-slate-100 p-5 sm:grid-cols-2 sm:p-7"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Complaint area</p><p className="mt-2 font-bold text-navy">{task.area}</p></div><div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Priority</p><p className="mt-2 font-bold capitalize text-navy">{task.priority}</p></div>{task.complaintReference && <div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Complaint reference</p><p className="mt-2 font-bold text-navy">{task.complaintReference}</p></div>}{task.routedDepartment && <div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Suggested department</p><p className="mt-2 font-bold text-navy">{task.routedDepartment}</p></div>}<div className="sm:col-span-2"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Verified work address</p><p className="mt-2 text-sm font-semibold leading-6 text-navy">{address}</p><p className="mt-1 text-xs text-slate-500">Post office: {task.postOffice || 'Not recorded'}</p><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(mapQuery)}`} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs font-extrabold text-blue-700">Open directions ↗</a></div></div>
...
export default function OrganisationStaffDashboard({ organisation, onBack }) {
  ...
  return <div className="min-h-screen bg-[#f5f7fb] text-ink"><div className="h-1.5 bg-gradient-to-r from-saffron via-white to-india-green" />
    ...
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex min-h-16 max-w-[1500px] items-center justify-between gap-4 px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><img src={organisation.logoUrl || logo} alt="" className="h-10 w-10 shrink-0 rounded-lg object-contain" /><div className="min-w-0"><strong className="block truncate text-sm text-navy">{organisation.organisationName}</strong><span className="text-xs text-slate-500">Staff workspace</span></div></div><div className="flex items-center gap-3"><button onClick={load} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-navy">Refresh</button><button onClick={onBack} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-navy">Website</button></div></div></header>
    <main className="mx-auto max-w-[1500px] p-4 sm:p-7 lg:p-9"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-orange-700">Field operations</p><h1 className="mt-2 text-3xl font-extrabold text-navy">My assigned tasks</h1><p className="mt-2 text-sm text-slate-500">View locations, complete the work, and provide photo evidence.</p></div><div className="flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm"><button onClick={() => { setView('active'); setSelectedId(activeTasks[0]?.id || null) }} className={`rounded-lg px-4 py-2 text-sm font-bold ${view === 'active' ? 'bg-navy text-white' : 'text-slate-600'}`}>Active ({activeTasks.length})</button><button onClick={() => { setView('completed'); setSelectedId(completedTasks[0]?.id || null) }} className={`rounded-lg px-4 py-2 text-sm font-bold ${view === 'completed' ? 'bg-india-green text-white' : 'text-slate-600'}`}>Done ({completedTasks.length})</button></div></div>
