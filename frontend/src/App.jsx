...
const slides = [
  { tag: 'A vision of people-first service', title: 'Sahai India: service with compassion', text: 'A conceptual poster celebrating citizens, volunteers, and the spirit of helping every community move forward together.', image: modiPoster, dark: true, note: 'Concept visual • Not an official endorsement' },
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#fffdf9] text-ink">
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8" aria-label="Main navigation">
        <a href="#" className="flex min-w-0 items-center gap-3"><img src={logo} alt="Sahai India" className="h-14 w-14 shrink-0 object-contain sm:h-16 sm:w-16"/><span className="border-l border-slate-200 pl-3"><strong className="block font-serif text-xl text-navy sm:text-2xl">Sahai India</strong><span className="block text-[9px] font-bold uppercase tracking-[.16em] text-slate-500 sm:text-xs">Help • Hope • Humanity</span></span></a>
      ...
      <section id="about" className="bg-slate-50 p-3 sm:p-5 lg:p-7" aria-label="Featured programmes">
        <div className="relative mx-auto min-h-[500px] max-w-[1440px] overflow-hidden rounded-[1.75rem] bg-navy shadow-xl sm:min-h-[560px]">
          {slides.map((item,i) => <article key={item.title} aria-hidden={slide !== i} className={`absolute inset-0 transition-opacity duration-700 ${slide === i ? 'z-10 opacity-100' : 'pointer-events-none opacity-0'}`}>
            {item.image ? <div className="absolute inset-0 bg-cover bg-[70%_center] sm:bg-center" style={{ backgroundImage: `url(${item.image})` }} /> : <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient}`}/>}<div className={`absolute inset-0 ${item.image ? 'bg-gradient-to-r from-[#071d43]/95 via-[#071d43]/45 to-transparent' : 'fine-grid'}`}/><div className="absolute -right-20 -top-20 h-96 w-96 rounded-full border-[70px] border-white/10"/><div className="absolute -bottom-32 right-[8%] h-80 w-80 rounded-full bg-white/10"/>
            <div className="relative z-10 flex min-h-[500px] items-center px-7 py-16 sm:min-h-[560px] sm:px-14 lg:px-20"><div className={`max-w-2xl ${item.dark ? 'text-white' : 'text-navy'}`}>
              ...
              {item.note && <p className="mt-5 text-xs font-semibold text-white/70">{item.note}</p>}
            </div></div>
          </article>)}
